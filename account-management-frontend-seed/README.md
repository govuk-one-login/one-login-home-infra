...
These placeholder resources are for storing config values used when the `account-management-frontend` CloudFormation
...
##### Stack Parameters
 - `AAMStackName` - The intended name of the AAM SAM Stack, defaults to `account-management-frontend`.
...
Get the KMS Key's Public Key and send it to the OP for registration of the RP.
 ...
 - Find key with Alias called `account-management-frontend-JwtSigningKey` and select it.
...
|------------------------------------------------------|------------------------------------------------|   
| `/account-management-frontend/Config/OIDC/Client/Id` | Client ID returned by OP after RP registration | 
| `/account-management-frontend/Config/GTM/Id`         | Google Tag Manager ID                          | 
...
|----------------------------------------------------------|--------------------------------------|   
| `/account-management-frontend/Config/Publishing/API/Key` | GOV.UK Services - Publishing API Key |

__N.B__ SecureString cannot be created with CloudFormation.

#### Step 5
Deploy the `account-management-frontend` SAM stack.

### Stack Outputs

| Type           | Name                                                        | Description                                                                                  |
|----------------|-------------------------------------------------------------|----------------------------------------------------------------------------------------------|
| Stack Export   | `authentication-account-management-seed-JwtSigningKeyId`    | ID of the JWT signing KMS key                                                                |
| Stack Export   | `authentication-account-management-seed-JwtSigningKeyAlias` | Alias of the JWT signing KMS key, normally `alias/account-management-frontend-JwtSigningKey` |
| SSM Parameter  | `/account-management-frontend/KMS/JwtSigningKey/Id`         | ID of the JWT signing KMS key                                                                |
| SSM Parameter  | `/account-management-frontend/KMS/JwtSigningKey/Alias`      | Alias of the JWT signing KMS key, normally `alias/account-management-frontend-JwtSigningKey` |
| SSM Parameter  | `/account-management-frontend/Config/OIDC/Client/Id`        | Config Placeholder for Step 3                                                                |
| SSM Parameter  | `/account-management-frontend/Config/GTM/Id`                | Config Placeholder for Step 3                                                                |
| Secret Manager | `/account-management-frontend/Config/Publishing/API/Key`    | Config Placeholder for Step 4                                                                |
